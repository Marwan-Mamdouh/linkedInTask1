...
class DealerControllerTest {
  ...
  void shouldGetDealer_whenExists() throws Exception {
    mockMvc
        .perform(
            get("/api/dealers/{id}", existingDealerId).header("X-Tenant-Id", tenantId.toString()))
  ...
  void shouldReturn404_whenDealerDoesNotExist() throws Exception {
    mockMvc
        .perform(
            get("/api/dealers/{id}", UUID.randomUUID()).header("X-Tenant-Id", tenantId.toString()))
        .andExpect(status().isNotFound());
  }
  ...
  void invalidTenantHeader_returns400() throws Exception {
    mockMvc
        .perform(get("/api/dealers").header("X-Tenant-Id", "not-a-uuid"))
        .andExpect(status().isBadRequest())
        .andExpect(jsonPath("$.error", is("Invalid or missing X-Tenant-Id")));
  }
}
