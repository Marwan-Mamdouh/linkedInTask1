...
class DealerServiceTest {
  ...
  void shouldThrowResourceNotFound_whenDealerNotFound() {
    ...
    assertThrows(
        ...
        () -> dealerService.getDealerByIdAndTenantId(dealerId));
    verify(dealerRepository).findByIdAndTenantId(dealerId, tenantId);
  }

  @Test
  void shouldUpdateDealer_whenValidInput() {
    UpdateDealerRequest request = new UpdateDealerRequest("New Name", "new@dealer.com", SubscriptionType.PREMIUM);
  ...
  void shouldCountDealersBySubscription() {
    ...
    when(dealerRepository.countDealersBySubscription(tenantId))
        .thenReturn(List.of(countBasic));

    Map<String, Long> counts = dealerService.getDealerSubscriptionCounts(tenantId);

    assertThat(counts)
        .containsEntry(SubscriptionType.BASIC.name(), 5L)
        .hasSize(1);
